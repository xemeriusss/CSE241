      ...
      programmableInterruptControllerSlaveDataPort(0xA1)
{
    ...
    interruptCounter = 0; // Initialize interrupt counter
...
uint32_t InterruptManager::DoHandleInterrupt(uint8_t interrupt, uint32_t esp)
{
    ...
    {
        // If interrupt number is 0x80, then it is a syscall. So, we need to call the syscall handler. esp comes from the CPUState struct
        // If not, then it is a hardware interrupt. So, we need to call the hardware interrupt handler
