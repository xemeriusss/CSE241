...
: InterruptHandler(manager, 0x21), // 0x21 is for keyboard interrupt
dataport(0x60),
commandport(0x64)
{
...
void KeyboardDriver::Activate()
{
    ...
    uint8_t status = (dataport.Read() | 1) & ~0x10; // set the last bit to 1 and the 5th bit to 0 because new bits
...
{
    uint8_t key = dataport.Read(); // read the key
    ...
    esp = (uint32_t)interruptManager->GetTaskManager()->Schedule((CPUState*)esp);
