...
public class Account {
    ...
    public String showFollowings(Account account) {
        ...
        String followings = "";
        LDLinkedList<Account> followingsList = account.getFollowing().ldlist();
        for (int i = 0; i < followingsList.size(); i++) {
            followings += followingsList.get(i).getUsername() + ", ";
        }
        return followings;
    }
    ...
    public String showFollowers(Account account) {
        String followers = "";
        LDLinkedList<Account> followersList = account.getFollowers().ldlist();
        for (int i = 0; i < followersList.size(); i++) {
            followers += followersList.get(i).getUsername() + ", ";
        }
        return followers;
    }
    ...
    public void likePost(Post post, int interactionId, Account account) {
        ...
        for(int i=0 ; i< post.getLikes().size() ; i++) {
            if(post.getLikes().get(i).getAccountId() == this.accountId) {
    ...
    public void unlikePost(Post post, int interactionId, Account account) {
        ...
        for(int i=0 ; i< post.getLikes().size() ; i++) {
            if(post.getLikes().get(i).getAccountId() == this.accountId) {
                ...
                post.getLikes().remove(post.getLikes().get(i));
                break;
            }
        }
        if(flag==0) {
    ...
    public void uncommentPost(Post post, int interactionId, Account account) {
        ...
        for(int i=0 ; i< post.getComments().size() ; i++) {
            if(post.getComments().get(i).getAccountId() == this.accountId) {
                flag=1; // if flag=1, it means that the user commented the post before
                post.getComments().remove(post.getComments().get(i)); // remove the comment from the post
                break;
            }
        }
        if(flag==0) {
    ...
    public void viewInteractions(Account account) {
        ...
        for(Post post : posts) {
            ...
            if(post.getLikes().size() != 0) {
                // get the likes of the post of lazy deletion linked list
                LDLinkedList<Like> likes = post.getLikes().ldlist();
            ...
            if(post.getComments().size() != 0) {
                // get the comments of the post of lazy deletion linked list
                LDLinkedList<Comment> comments = post.getComments().ldlist();
                System.out.println("The post has " + comments.size() + " comment(s).");
                for(Comment comment : comments) {
                    System.out.println("Comment " + comment.getInteractionId()+ ": '" + comment.getUsername() + "' said '" + comment.getContent() + "'.");
    ...
    public void block(Account account) {
        ...
        // unfollow the account that the user blocked
    ...
    public String showBlockedAccounts(Account account) {
        String blocked = "";
        LDLinkedList<Account> blockedList = account.getBlockedAccounts().ldlist();
        for (int i = 0; i < blockedList.size(); i++) {
            blocked += blockedList.get(i).getUsername() + ", ";
        }
        return blocked;
    }

    /**
     * If the user is logged in, then print out the history of the user.
     */
